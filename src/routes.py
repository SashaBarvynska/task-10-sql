...
def delete_student(student_id: int) -> wrappers.Response:
    with get_session() as session:
        ...
        if not student:
            return {'error': f'Student by {student_id} not found.'}, HTTPStatus.NOT_FOUND
        else:
            student_repository.delete_student_by_id(student_id)
            return {'message': f'Student by {student_id} has been deleted.'}, HTTPStatus.OK
...
def delete_course(student_id: int) -> wrappers.Response:
    with get_session() as session:
        ...
        student_course_repository = StudentCourseRepository(session)
        course = student_course_repository.get_course_to_student_by_id(course_id, student_id)
        if not course:
...
def get_groups() -> wrappers.Response:
    with get_session() as session:
        ...
        groups_with_max_students = GroupRepository(session).get_groups_with_max_students(max_students)
        dict_students = list(map(lambda group: asdict(Group(group.id, group.name)), groups_with_max_students))
        return jsonpickle.encode(dict_students), HTTPStatus.OK
